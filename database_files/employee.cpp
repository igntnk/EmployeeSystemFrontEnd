...
void Employee::removeTask(int number){delete m_task[number]; m_task.erase(m_task.begin()+number);}
void Employee::setRank(Rank* rank){delete m_rank; m_rank=rank;}
void Employee::addDocument(Document* document){m_document.push_back(document);}
void Employee::removeDocument(int number){delete m_document[number]; m_document.erase(m_document.begin()+number);}
void Employee::addPayment(Payment* payment){m_payment.push_back(payment);}
void Employee::removePayment(int number){delete m_payment[number]; m_payment.erase(m_payment.begin()+number);}
